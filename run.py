...
def data_handle(params, permutation, makespan, version):
    # 存储结果
    conn = sqlite3.connect('2.db')  # 连接到 SQLite 数据库（如果不存在则会自动创建）
    ...
    # 插入数据
    cursor.execute(
        "INSERT INTO results (n, m, seed, Pmax, Smin, Smax, sigma_min, sigma_max, pls, a, x, b, tmax, permutation, makespan, version, time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)",
        (int(params['tuple'][0]), int(params['tuple'][1]), int(params['tuple'][2]), int(params['Pmax']),
         int(params['Smin']), int(params['Smax']),
         int(params['sigma_min']), int(params['sigma_max']), float(params['pls']), int(params['a']),
         int(params['x']),float(params['b']), int(params['tmax']),
...
def evolution_algorithm(params):
    ...
    for i in range(runs_per_instance):
        result = main.DIWO(Pmax, Smin, Smax, sigma_min, sigma_max, pls, jobs,x,a,b, tmax, cost_function)
